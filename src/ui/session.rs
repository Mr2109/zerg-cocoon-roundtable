        ...
        rt2.spawn(async move {
                ...
                .await
            {
                    ...
                    if summary.completed {
                        let _ = db
                            .add_message(
                                ...
                                "系统",
                                &format!(
                                    "✅ 讨论完成：{}/{} Block 全部跑完——可前往 📖 章节查看正文",
                                    summary.blocks_done, summary.blocks_total
                                ),
                                "system",
                                "",
                            )
                ...
                Err(e) => {
                    ...
                    log::error!("会话 {} 引擎错误: {}", sid_owned, msg);
...
fn make_ai(prov: &str) -> Box<crate::engine::discussion::BoxAi> {
    if crate::ui::ai_connected() {
        ...
        if p.is_empty() || !crate::ui::MODELS.contains(&p) {
            log::warn!("provider '{}' 不在候选——回退 env 默认模型", p);
...
pub fn session_view(app: &mut RoundtableApp, ui: &mut egui::Ui, sid: &str) {
    ...
    let messages = app
        .rt
        .block_on(app.db.get_all_messages(sid))
        .ok()
        .unwrap_or_default();
    ...
    egui::Panel::left("blocks")
        ...
        .show(ui, |ui| {
            ...
            for (i, blk) in tmpl.blocks.iter().enumerate() {
                let locked = templates
                    .iter()
                    .any(|t| t.locked == 1 && t.block_name.as_deref() == Some(&blk.name));
    ...
    egui::Panel::right("fields")
        ...
        .show(ui, |ui| {
            ...
            for t in &templates {
                let key = format!(
                    "{}_{}",
                    t.block_index.unwrap_or(0),
                    t.field_name.clone().unwrap_or_default()
                );
    ...
    egui::CentralPanel::default().show(ui, |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                ui.label(
                    RichText::new(format!("{}px", app.msg_px as i32))
                        .weak()
                        .small(),
                );
                if ui.button("A−").clicked() {
