...
pub async fn review_chapter_content(
    ...
    let text = db
        .get_chapter_content_text(chapter_id)
        .await
    ...
    // 清旧
    db.clear_chapter_reviews(chapter_id)
        .await
        .map_err(|e| e.to_string())?;
    db.clear_review_feedback(chapter_id)
        .await
        .map_err(|e| e.to_string())?;
    ...
    let conflict_result = ai
        .chat(
            &[
                sys_msg("你严谨审查设定一致性，只输出具体冲突，不输出废话。"),
                user_msg(&conflict_prompt),
            ],
            2048,
        )
        .await
    ...
    if !conflict_result.contains("无冲突") {
        for line in conflict_result.lines() {
            ...
            let severity = if line.contains("世界观") || line.contains("角色") {
                "critical"
            } else {
                "major"
            };
            db.add_review(chapter_id, "设定冲突", severity, 0, line, "")
                .await
                .map_err(|e| e.to_string())?;
            let fid = db
                .add_review_feedback(sid, chapter_id, "设定冲突", severity, line, "")
                .await
                .map_err(|e| e.to_string())?;
            fb_ids.push(fid);
            results.push(ReviewFinding {
                pipeline: "设定冲突".into(),
    ...
    let lang_result = ai
        .chat(
            &[
                sys_msg("你是专业文字编辑，严格逐项检查，只输出问题。"),
                user_msg(&lang_prompt),
            ],
            2048,
        )
        .await
    ...
    if !lang_result.contains("语言质量合格") {
        for line in lang_result.lines() {
            ...
            let severity = if line.contains("错别字") || line.contains("语病") {
                "major"
            } else {
                "minor"
            };
            ...
            db.add_review(chapter_id, "语言质量", severity, 0, &issue_text, line)
                .await
                .map_err(|e| e.to_string())?;
            let fid = db
                .add_review_feedback(sid, chapter_id, "语言质量", severity, &issue_text, line)
                .await
                .map_err(|e| e.to_string())?;
            fb_ids.push(fid);
            results.push(ReviewFinding {
    ...
    if auto_rewrite && !rewrite_issues.is_empty() {
        ...
        if let Some(rw) = rewritten {
            ...
            for fid in &fb_ids {
                db.mark_feedback_rewritten(*fid, rw.as_str())
                    .await
                    .map_err(|e| e.to_string())?;
            }
            results.push(ReviewFinding {
                pipeline: "自动重写".into(),
                severity: "info".into(),
                issue: format!("已按 {} 条问题重写", rewrite_issues.len()),
            });
        }
    }
    Ok(results)
...
async fn rewrite_chapter(
    ...
    db.update_chapter_content(sid, chapter_id, &reply.content)
        .await
        .map_err(|e| e.to_string())?;
    db.clear_content_chunks(chapter_id)
        .await
        .map_err(|e| e.to_string())?;
    ...
    for (i, para) in paragraphs.iter().enumerate() {
        db.add_content_chunk(chapter_id, i as i64, para)
            .await
            .map_err(|e| e.to_string())?;
    }
...
pub async fn build_review_lessons(db: &Db, sid: &str) -> Result<String, String> {
    let lessons = db
        .get_review_feedback(sid, "rewritten", 10)
        .await
        .map_err(|e| e.to_string())?;
    if lessons.is_empty() {
...
mod tests {
    ...
    async fn setup(sid: &str, db: &Db) {
        db.create_session(sid, "评审测试", "玄幻", "长篇", "zerg", "novel")
            .await
            .unwrap();
        db.upsert_template(
            sid,
        ...
        .await
        .unwrap();
        db.create_chapter(sid, 1, 1, "初章", "林风踏入宗门。")
            .await
            .unwrap();
        let ch = &db.get_chapters(sid).await.unwrap()[0];
        db.add_content_chunk(ch.id, 0, "林风走进宗门大殿，看见许多弟子。")
            .await
            .unwrap();
        db.add_content_chunk(ch.id, 1, "他握紧拳头，暗暗发誓要变强。")
            .await
            .unwrap();
    }
    ...
    async fn conflict_triggers_rewrite() {
        ...
        let findings = review_chapter_content(&db, &ai, "v1", ch.id, true)
            .await
            .unwrap();
        assert!(
        ...
        assert!(
            findings.iter().any(|f| f.pipeline == "自动重写"),
            "自动重写触发"
        );
        // 正文被覆盖
    ...
    async fn clean_passes_through() {
        ...
        let findings = review_chapter_content(&db, &ai, "v2", ch.id, true)
            .await
            .unwrap();
    ...
    async fn lessons_built() {
        ...
        setup("v3", &db).await;
        db.add_review_feedback("v3", 1, "设定冲突", "critical", "飞船乱入", "删掉飞船")
            .await
            .unwrap();
        // 标记 rewritten
